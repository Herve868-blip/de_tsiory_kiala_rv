...
class SessionCalendrier(models.Model):
    ...
    def _check_regle_complete(self):
        for rec in self:
            ...
            coeffs_utilises = self.env['de_inscae.session_calendrier_fi_matiere'].search([
                ('session_calendrier_fi_id.session_calendrier_id', '=', rec.id),
            ]).mapped('coefficient_id')
            coeffs_utilises |= self.env['de_inscae.matiere_dispo_session_fc'].search([
                ('session_calendrier_fc_id.session_calendrier_id', '=', rec.id),
            ]).mapped('coefficient_id')

            coeffs_couverts = rec.regle_admission_id.detail_ids.mapped('coefficient_id')
            coeffs_manquants = coeffs_utilises - coeffs_couverts
            ...
            if coeffs_manquants:
                ...
                raise ValidationError(
                    ...
                    f"utilisés dans cette session : {manquants_str}."
                )
...
class SessionCalendrierFC(models.Model):
    ...
    session_calendrier_id = fields.Many2one('de_inscae.session_calendrier', required=True, ondelete='cascade')
    session_parente_fc_id = fields.Many2one('de_inscae.session_parente_fc', string="Session Parente FC", required=True)
    ...
    session_calendrier_fc_prec_id = fields.Many2one('de_inscae.session_calendrier_fc', string="Session FC Précédente")
    ...
    def _recalculer_tout(self):
        ...
        for groupe in self.groupe_fc_ids:
            if not groupe.notes_verrouillees:
                continue
            groupe._recalculer_notes(session_cal, regle, remplacer_tentatives=True)

        # Recalculer résultats session
        for inscription in self.etudiant_session_fc_ids:
