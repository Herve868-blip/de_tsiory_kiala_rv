...
class Groupe(models.Model):
    ...
    planning_ids = fields.One2many('de_inscae.planning', 'groupe_id', string="Planning")
    ...
    def action_ajouter_planning(self):
        self.ensure_one()
        return {
            ...
            'view_mode': 'form',
            'target': 'new',
            'views': [(self.env.ref('de_inscae.view_planning_form').id, 'form')],
            'context': {'default_groupe_id': self.id},
        }
...
class GroupeFI(models.Model):
    ...
    def action_ajouter_planning(self):
        return self.groupe_id.action_ajouter_planning()
...
class GroupeFC(models.Model):
    ...
    matiere_dispo_session_fc_id = fields.Many2one(
        'de_inscae.matiere_dispo_session_fc',
        string="Matière de la Session",
        required=True,
        domain="[('session_calendrier_fc_id', '=', session_calendrier_fc_id)]"
    )
    ...
    def _check_coherence_session(self):
        for rec in self:
            if rec.matiere_dispo_session_fc_id.session_calendrier_fc_id != rec.session_calendrier_fc_id:
                raise ValidationError("La matière sélectionnée n'appartient pas à la session FC de ce groupe.")
    ...
    def _get_detail_regle(self, regle):
    ...
    def _recalculer_notes(self, session_cal, regle, remplacer_tentatives=False):
        detail = self._get_detail_regle(regle)
        ...
        for membre in self.membre_ids:
            ...
            resultat = membre._determiner_resultat(moyenne, detail)
            membre.write({'resultat': resultat})

            if resultat != 'deliberation':
                membre._creer_ou_remplacer_tentative(
                    etudiant, matiere, session_cal,
                    self.prof_id, moyenne,
                    resultat == 'admis',
                    remplacer=remplacer_tentatives,
                )
    ...
    def _recalculer_apres_ponderation(self):
        ...
        inscriptions = self.membre_ids.mapped('etudiant_session_fc_id')
